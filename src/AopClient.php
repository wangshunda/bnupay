...
class AopClient {
	...
	//使用文件读取文件格式，请只传递该值
	public $bnupayPublicKey = null;

	//使用读取字符串格式，请只传递该值
	public $bnupayrsaPublicKey;
	...
	protected $bnupaySdkVersion = "bnupay-sdk-php-20191101";

	public function generateSign($params, $signType = "MD5",$request) {
		return ($this->sign($this->getSignContent($params), $signType,$request));
	}
	...
	protected function sign($data, $signType = "MD5",$request) {
		$priKey=$this->md5PrivateKey;
		if ("RSA2" == $signType) {
			...
			$data2sign=explode(",",$data);

			$signstring="";
			$signstringtemp=explode(":",$data2sign[1]);
			$signstring=str_replace("\"","",$signstringtemp[0])."=".str_replace("\"","",$signstringtemp[1]);

			$signstringtemp=explode(":",$data2sign[2]);
			$signstring.="&".str_replace("\"","",$signstringtemp[0])."=".str_replace("\"","",$signstringtemp[1]);

			$signstringtemp=explode(":",$data2sign[3]);
			$signstring.="&".str_replace("\"","",$signstringtemp[0])."=".str_replace("\"","",$signstringtemp[1]);

			$signstringtemp=explode(":",substr($data2sign[4],0,stripos($data2sign[4],"}")));
			$signstring.="&".str_replace("\"","",$signstringtemp[0])."=".str_replace("\"","",$signstringtemp[1]);

			$signstring.='&return_url='.$request->getReturnUrl().'&notify_url='.$request->getNotifyUrl().$priKey;
	...
	public function sdkExecute($request) {
		...
		$params['bnupay_sdk'] = $this->bnupaySdkVersion;
	...
	*/
	public function pageExecute($request,$httpmethod = "POST") {
		...
		//签名
		$totalParams["sign"] = $this->generateSign($totalParams, $this->signType,$request);
	...
	protected function buildRequestForm($para_temp) {
		...
		$sHtml = "<form id='bnupaysubmit' name='bnupaysubmit' action='".$this->gatewayUrl."' method='POST'>";
		while (list ($key, $val) = each ($para_temp)) {
			...
			{
				$val = str_replace("\"","",$val);
				$arraybig = explode(",", $val);
				$signstring="";
				while(list ($key, $val) = each ($arraybig))
				{
					list ($keydetail, $valdetail) = explode(":",$val);
					if($keydetail !="biz_content" && $keydetail !="{product_code" && $keydetail !="sign")
					{
						$valdetail=str_replace("}","",$valdetail);
						$sHtml.= "<input type='hidden' name='".$keydetail."' value='".$valdetail."'/>";
					}
				}
			}
        }
		...
		$sHtml = $sHtml."<script>document.forms['bnupaysubmit'].submit();</script>";
	...
	public function execute($request, $authToken = null, $appInfoAuthtoken = null) {
		...
		$sysParams["bnupay_sdk"] = $this->bnupaySdkVersion;
	...
	function verify($data, $sign, $rsaPublicKeyFilePath, $signType = 'RSA') {
		...
		if($this->checkEmpty($this->bnupayPublicKey)){

			$pubKey= $this->bnupayrsaPublicKey;
...
/** 
	...
	public function encryptAndSign($bizContent, $rsaPublicKeyPem, $rsaPrivateKeyPem, $charset, $isEncrypt, $isSign, $signType='RSA') {
		...
		if ($isEncrypt && $isSign) {
			...
			$response = "<?xml version=\"1.0\" encoding=\"$charset\"?><bnupay><response>$encrypted</response><encryption_type>RSA</encryption_type><sign>$sign</sign><sign_type>$signType</sign_type></bnupay>";
			return $response;
		}
		// 加密，不签名
		if ($isEncrypt && (!$isSign)) {
			$encrypted = $this->rsaEncrypt($bizContent, $rsaPublicKeyPem, $charset);
			$response = "<?xml version=\"1.0\" encoding=\"$charset\"?><bnupay><response>$encrypted</response><encryption_type>$signType</encryption_type></bnupay>";
			return $response;
		}
		// 不加密，但签名
		if ((!$isEncrypt) && $isSign) {
			$sign = $this->sign($bizContent, $signType);
			$response = "<?xml version=\"1.0\" encoding=\"$charset\"?><bnupay><response>$bizContent</response><sign>$sign</sign><sign_type>$signType</sign_type></bnupay>";
			return $response;
		}
		// 不加密，不签名
	...
	public function checkResponseSign($request, $signData, $resp, $respObject) {
		...
		if (!$this->checkEmpty($this->bnupayPublicKey) || !$this->checkEmpty($this->bnupayrsaPublicKey)) {
			...
			if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
				...
				$checkResult = $this->verify($signData->signSourceData, $signData->sign, $this->bnupayPublicKey, $this->signType);
